...
# Akto's guardrail is intermittent and fails OPEN: for byte-identical input it
# returns Allowed=true on some calls and a block on others, sometimes varying
# call-to-call within seconds (measured 0/4 then 8/8 on the same probe minutes
# apart, and BB./.BB inside single windows). An "allow" therefore carries much
# less information than a "block". Re-ask on allow and treat any block as
# authoritative. Costs extra latency on allowed requests, so it is bounded and
# only applies to the blocking (SYNC_MODE) path.
GUARDRAIL_VERDICT_RETRIES = int(os.getenv("GUARDRAIL_VERDICT_RETRIES", "0"))
# Drop `stream` from the guardrail's copy of the body. Akto's detector returns
# allow whenever that key is present (measured; see _drop_verdict_blind_keys).
VERDICT_DROP_STREAM_KEY = os.getenv("VERDICT_DROP_STREAM_KEY", "true").strip().lower() == "true"
# Body keys that suppress Akto's prompt-injection detection. Measured by
# replaying real captured verdict envelopes, 4 trials each:
#     as captured (with stream + tools)      -> allowed 4/4
#     same, tools removed                    -> BLOCKED 4/4
#     same, stream removed only              -> allowed
# Both are metadata about the call rather than content to judge, so neither
# belongs in the verdict copy. Ingestion keeps them.
# Akto's guardrail reads message content only as a plain string. Anthropic-style
# content BLOCKS (content: [{"type":"text","text":...}]) are not read at all, so
# every Claude Code request slipped through. Measured on a real captured Claude
# Code verdict envelope, 4 trials each:
#     content as list of blocks   -> allowed 4/4
#     same, flattened to a string -> BLOCKED 4/4
# Flattened for the verdict copy only; ingestion keeps the original structure.
...
class GuardrailsHandler(CustomLogger):
    def __init__(self):
        ...
        # connector's own blocks would be recorded twice.
    ...
    @staticmethod
    def _strip_stream_from_envelope(payload: dict) -> dict:
        """Remove `stream` from the built envelope's mirrored request body.

        Applies to the guardrail verdict call only; ingestion envelopes keep it.
        """
        if not VERDICT_DROP_STREAM_KEY:
            return payload
        try:
            rp = json.loads(payload.get("requestPayload") or "{}")
            body = rp.get("body")
            if not isinstance(body, dict):
                return payload
            changed = False
            for k in VERDICT_DROP_BODY_KEYS:
                if k in body:
                    body.pop(k, None)
                    changed = True
            ...
            if changed:
                out = dict(payload)
                out["requestPayload"] = json.dumps(rp)
                return out
        except (json.JSONDecodeError, TypeError, AttributeError) as e:
            logger.warning(f"Could not strip stream from verdict payload: {e}")
        return payload

    @staticmethod
    def _drop_verdict_blind_keys(view: dict) -> dict:
        """Remove body keys that stop Akto's guardrail evaluating the prompt.

        Measured against the live service, 4 trials each, same window:
            {model, messages}                  -> blocked 4/4
            + stream: false                    -> ALLOWED 4/4
            + tools: [] / [{...}]              -> blocked 4/4
            + max_tokens                       -> blocked 4/4
        So the mere presence of `stream` suppresses detection. Every LiteLLM
        request carries it, which is why prompt-injection blocking looked broken.
        Only the verdict copy is touched - the request forwarded to the model and
        the payload sent for ingestion both keep `stream` intact.
        """
        if not VERDICT_DROP_STREAM_KEY or "stream" not in view:
            return view
        cleaned = dict(view)
    ...
    def _validation_view(cls, data: dict) -> dict:
        ...
        if not cls._should_narrow(data):
            return cls._drop_verdict_blind_keys(data)
        msgs = data["messages"]
        ...
        if VALIDATE_LAST_USER_MESSAGE_ONLY == "all_user":
            ...
            trimmed["tools"] = []
            trimmed = cls._drop_verdict_blind_keys(trimmed)
            logger.info(
    ...
    async def call_guardrails_validation(self, data: dict, call_type: str, user_api_key_dict: Optional[UserAPIKeyAuth] = None, kwargs: Optional[dict] = None) -> Tuple[bool, str, Optional[str]]:
        ...
        http_proxy_payload = self.build_payload(self._validation_view(data), call_type, None, user_api_key_dict, kwargs=kwargs)
        # build_payload hardcodes "stream": data.get("stream", False), so dropping
        # the key from the view is not enough - it has to come off the built
        # envelope. See _drop_verdict_blind_keys for why `stream` matters.
        http_proxy_payload = self._strip_stream_from_envelope(http_proxy_payload)

        # An "allow" from this service is weak evidence (it fails open on internal
        # errors), so re-ask; a block on any attempt is authoritative. Only the
        # blocking path pays the extra latency.
        attempts = 1 + max(0, GUARDRAIL_VERDICT_RETRIES) if SYNC_MODE else 1
        last: Tuple[bool, str, Optional[str]] = (True, "", None)
        for attempt in range(1, attempts + 1):
            try:
                ...
                if response.status_code != 200:
                    ...
                    return True, "", None
                last = self.parse_guardrails_result(response.json())
            ...
            if not allowed:
                if attempt > 1:
                    logger.warning(
                        f"Guardrail blocked only on attempt {attempt}/{attempts} - the service "
                        f"returned allow for the same payload on earlier attempts"
                    )
                return last
        if attempts > 1:
            logger.info(f"Guardrail allowed after {attempts} attempts")
        return last
