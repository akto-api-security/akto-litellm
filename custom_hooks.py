...
# Judge only the newest user turn, not the agent scaffolding (see _validation_view).
VALIDATE_LAST_USER_MESSAGE_ONLY = os.getenv("VALIDATE_LAST_USER_MESSAGE_ONLY", "true").lower() == "true"
...
class GuardrailsHandler(CustomLogger):
    ...
    def _merge_redacted_last_user(self, data: dict, modified_payload) -> dict:
        """Apply Akto's redaction to the newest user turn only, preserving history."""
        try:
            payload_obj = json.loads(modified_payload) if isinstance(modified_payload, str) else modified_payload
            body = payload_obj.get("body") or {}
            redacted_msgs = body.get("messages") or []
            if not redacted_msgs:
                return data
            redacted_last = redacted_msgs[-1]
            msgs = list(data.get("messages") or [])
            for i in range(len(msgs) - 1, -1, -1):
                if msgs[i].get("role") == "user":
                    msgs[i] = redacted_last
                    merged = dict(data)
                    merged["messages"] = msgs
                    logger.info(f"Applied Akto redaction to newest user turn (history preserved: {len(msgs)} messages)")
                    return merged
            return data
        except Exception as e:
    ...
    async def validate_and_block(self, data: dict, call_type: str, user_api_key_dict: Optional[UserAPIKeyAuth] = None, kwargs: Optional[dict] = None) -> dict:
        try:
            ...
            if modified_payload:
                if VALIDATE_LAST_USER_MESSAGE_ONLY:
                    # We only showed Akto the newest user turn, so ModifiedPayload
                    # contains only that turn. apply_redaction() swaps in the whole
                    # body wholesale, which would drop the rest of the conversation -
                    # so splice the redacted turn back into the original messages.
    ...
    @staticmethod
    def _validation_view(data: dict) -> dict:
        """What the guardrail should judge: the user's newest turn only.

        Coding agents send a large system prompt and a full tool catalogue on
        every request. Handing all of that to the guardrail means the policy
        matches the agent's own scaffolding, so every request is rejected no
        matter what the user typed. Ingestion still records the full payload -
        this narrowing applies only to the verdict call.
        """
        if not VALIDATE_LAST_USER_MESSAGE_ONLY:
            return data
